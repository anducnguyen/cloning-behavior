...
int main(int argc, char **argv)
{
	...
	while (ros::ok()) {
		...
		DerivativeFilter car_vx(px, Tf, dt);	// the second arg: Derivative Filter's time constant Ts
		car_vx.calculate(x_vx);
		car.setVx(car_vx.get_output());

		DerivativeFilter car_vy(py, Tf, dt);	// the second arg: Derivative Filter's time constant Ts
		car_vy.calculate(x_vy);
		car.setVy(car_vy.get_output());

		DerivativeFilter car_vz(pz, Tf, dt);	// the second arg: Derivative Filter's time constant Ts
		...
		double steerDeg = -u_opt[0] * RAD2DEG;
