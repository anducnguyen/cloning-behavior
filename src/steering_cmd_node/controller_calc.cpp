...
/*
  ...
  */      
        ...
        if (Flag_motor_start == 0) {
			...
			std::uniform_int_distribution<std::mt19937::result_type> dist1(-10,10); // distribution in range [1, 6]
			std::uniform_int_distribution<std::mt19937::result_type> dist2(20,80); // distribution in range [1, 6]
			//~ std::uniform_int_distribution<std::mt19937::result_type> dist2(100,600); // distribution in range [1, 6]

			// magnitude_speed_relay = (rand() % 20 - 10.0)/10.0 ;
			magnitude_speed_relay = rand() % 25 - 10.0;

			//magnitude_speed_relay = dist1(rng);
            ...
            // speed_pwm_cmd = 10 + speed_relay;
   			//fprintf(stdout,"Print mag, freq, loop, speed_relay = (%d, %d, %d, %.1f) \n", magnitude_speed_relay, freq_speed_relay, loop_speed_relay, speed_relay);
			
			// speed_ref = 1.5 + speed_relay;
			speed_ref = 1.5;
            ...
            a_ref = speed_ref;
            //a_ref = 0.1 + speed_relay;
            //~ speed_pwm_cmd = -(22.5 * a_ref - 3 * 6.97);
            //~ speed_pwm_cmd = (27.3 * a_ref + 8.2*current_speed);
            // speed_pwm_cmd = 15;

            		//~ fprintf(stdout,"a_ref = %.f\n",a_ref);
