...
double motor_speed = 1500, motor_steer = 1500;
...
int main(int argc, char **argv)
{
	...
	ros::Subscriber sub4 = n.subscribe(stateEKF_topic, 50, chatterCallback_StateEKF);
	...
	double dt0 = 1 / 25; 					// sampling time
	ros::Rate loop_rate(1 / dt0);			// Hz
	ros::WallTime time_check_start, time_check_end;
	ros::Time time_ros_now;
	double time_check_calc, dt;
	...
	double steer_pwm_cmd = 0.0, speed_pwm_cmd = 0.0, current_speed = 0.0;
	double steer_current, a_ref;
	...
	double lane_u = 0, lane_v = 0, theta_d_lane = 0, theta_e_lane = 0;
	...
	double D_theta_e_temp = 0, D_ye_temp = 0;

	SXDict nlp;
	std::map<std::string, DM> args;
	CarParams Cparams;
	nlp = mpcFormulation(Cparams);
	args = mpcConstraints(Cparams);
	// Solver's options
	Dict opts;
	opts["ipopt.tol"] = 1e-7;
	...
	vector<double> u_opt(1);

	DataIdent dataIdent;
	dataIdent.LoadDataFromCSV("/home/suzlab/Tuan/NUMiniAutoCar2/RC_car_ROS_MoCap/Calculating_PC/src/controller_calc/Path_Info/Data_ident.csv");
	vector<double> Cparams_ident;
	int data_local_radius = 1;
	...
	while (ros::ok()) {
		...
		// EKF 
		car.setX(xEKF);		car.setY(yEKF);		car.setZ(pos_z);	car.setYaw(psiEKF);
		px = car.getX();	py = car.getY();	pz = car.getZ();	car_yaw = car.getYaw();

		// ==============================================================================================
		// The velocity is calculated using Low-pass Filter
		DerivativeFilter car_vx(px, Tf, dt);	car_vx.calculate(x_vx);		car.setVx(car_vx.get_output());
		DerivativeFilter car_vy(py, Tf, dt);	car_vy.calculate(x_vy);		car.setVy(car_vy.get_output());
		DerivativeFilter car_vz(pz, Tf, dt);	car_vz.calculate(x_vz);		car.setVz(car_vz.get_output());
		...
		u = lane_u;			y_e = lane_v;			theta_e = theta_e_lane;

		// Calculate theta_e_dot and y_e_dot using derivative filter with
		// with the filter's time constant of Tf
		DerivativeFilter D_theta_e(theta_e, Tf, dt);	D_theta_e.calculate(D_theta_e_temp);	theta_e_dot = D_theta_e.get_output();
		DerivativeFilter D_ye(y_e, Tf, dt);				D_ye.calculate(D_ye_temp);				y_e_dot = D_ye.get_output();

		// Look Ahead to estimate Curvature
		int Nkappa = 100;
		MatrixXd kappaMtx(Nkappa, 1), uMtx(Nkappa, 4);		// 4 = 3+1: 3rd order polynomial
		double u_pred = lane_center.points[cache_to_g.prev_index].u;
		double u_pred_last = u_pred, u0_unwrapped = u_pred + cache_to_g.lap * lane_center.length;
		double u_predicted, kappa0 = lane_center.points[cache_to_g.prev_index ].kappa;

		for (int i = 0; i < Nkappa; i++) {			
			u_pred = lane_center.points[i + cache_to_g.prev_index % lane_center.size].u;
			u_predicted = unwrap_u(u_pred, u_pred_last, lane_center.length, cache_to_g.lap);
			u_pred_last = u_pred;

			kappaMtx << lane_center.points[i + cache_to_g.prev_index % lane_center.size].kappa - kappa0;
			uMtx << pow(u_predicted-u0_unwrapped, 3), pow(u_predicted - u0_unwrapped, 2), u_predicted - u0_unwrapped;	// 3rd order polynomial
		}
		MatrixXd kappa_coeff;
		kappa_coeff = LeastMeanSquare(kappaMtx, uMtx);	// predicted kappa is: [(u_predicted-u0)^3, (u_predicted-u0)^2, u_predicted-u0)] * kappa_coeff + kappa0. This is to make sure that the approximated current kappa = current kappa
		...
		double ahead_theta_e = 0.0, ahead_theta_d = 0.0, ahead_lane_u = 0.0, ahead_lane_v = 0.0;

		static CacheToFrenet cache_to_g_ahead;
		cache_to_g_ahead = lane_center.GetFrenet(ahead_x, ahead_y, car.getYaw(), ahead_lane_u, ahead_lane_v, ahead_theta_e, cache_to_g_ahead);
		...
		// cout << dataIdent.data_all<<endl;
		// cout <<"u, lane_u: "<<u<<','<<lane_u<<endl;
		// ros::WallTime time_check11 = ros::WallTime::now();
		// Cparams_ident = ParamIdentCalc(dataIdent, u, data_local_radius);
		Cparams_ident = dataIdent.ParamIdentCalc(u, data_local_radius);
		Cparams.SetParams(Cparams_ident);		
		// cout << "Cparams: "<< Cparams_ident<<endl;
		// ros::WallTime time_check12 = ros::WallTime::now();
		// double time_check1 = (time_check12 - time_check11).toNSec()*1e-6;
		// ROS_INFO_STREAM("Exectution time (ms): " << time_check1);
		
		// cout <<"Params: "<< Cparams_ident<<endl;
		...
		// }
		// else {
		steer_current = pwm2steerDeg(motor_speed)*DEG2RAD;
		// cout << motor_steer << endl;
		// steer_current = 0;

		// cout<<"steer current [deg]: "<< steer_current*RAD2DEG<<endl;
		ros::WallTime time_check21 = ros::WallTime::now();
		// vector<double> stateMPC{y_e, y_e_dot, theta_e, theta_e_dot, steer_current};
		vector<double> stateMPC{ y_e, y_e_dot, theta_e, theta_e_dot };

		cout << "========================" << endl;
		// fprintf(stdout, "ye, yedot, thetaE, thetaEdot, steer: (%.3f, %.3f, %.3f, %.3f, %.3f)\n", y_e, y_e_dot, theta_e, theta_e_dot, steer_current);
		u_opt = mpcCalculation(stateMPC, Cparams, args, nlp, opts);
		// cout <<"uopt: "<<u_opt<<endl;
		// u_opt[0] = 1.0;
		ros::WallTime time_check22 = ros::WallTime::now();
		double time_check2 = (time_check22 - time_check21).toNSec()*1e-6;
		// ROS_INFO_STREAM("Exectution time (ms): " << time_check2);

		double steerDeg = -u_opt[0] * RAD2DEG;	// when identifying the map steer-pwm, the sign was reversed.

		steer_pwm_cmd = (steerDeg2pwm(steerDeg));
		cout << motor_steer << "," << steerDeg2pwm(steerDeg) << endl;

		// steer_pwm_cmd = 0;
		// cout << "steer current [deg]: " << steer_current * RAD2DEG << " uopt: " << u_opt << " steerPWM: " << steer_pwm_cmd << endl;
		// cout << "========"<<endl;
		// cout <<u_opt<<endl;
		// steer_pwm_cmd = 0;
		...
		a_ref = speed_PIDcontrol.calculate(current_speed, speed_ref, x_PID_speed, dt, Flag_motor_start);
