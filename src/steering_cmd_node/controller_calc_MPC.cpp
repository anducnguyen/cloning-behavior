...
double motor_speed=1500, motor_steer=1481;
...
int main(int argc, char **argv)
{
    ...
    ros::Rate loop_rate(1/dt0);	// 100 Hz
    ...
    double steer_pwm_cmd = 0.0, steerPWMold = 1500;
    ...
    while(ros::ok()) {
        ...
        DerivativeFilter D_theta_e(theta_e,Tf,dt);
        D_theta_e.calculate(D_theta_e_temp);
        theta_e_dot = D_theta_e.get_output();
        
        DerivativeFilter D_ye(y_e,Tf,dt);
        ...
        // else {
			...
			ros::WallTime time_check1 = ros::WallTime::now();
			...
			// u_opt[0] = 1.0;
			ros::WallTime time_check2 = ros::WallTime::now();
			double time_check = (time_check2-time_check1).toNSec()*1e-6;
			ROS_INFO_STREAM("Exectution time (ms): " << time_check);
